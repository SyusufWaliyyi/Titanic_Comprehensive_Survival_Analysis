...
|---|---|---|---|
| All columns | Sentinel string stored instead of NULL -- invisible to `.isnull()` | Widespread | Replace with `np.nan` before any type conversion |
...
+-- titanic_analysis.png           # Composite output chart
+-- README.md                      # This file
...
| **1 -- Load & Inspect** | 4 | Read CSV, `.info()`, raw unique values (exposes sentinel strings), null audit before and after |
...
assert df[critical_cols].isnull().sum().sum() == 0, 'Nulls remain in critical columns'
```
...
|---|---|---|
| 1 | **Sex is the strongest survival predictor** (74.2% female vs 18.9% male) | The women and children first protocol was enforced with striking consistency |
